...
Zombie::Zombie(Vector3 position1, int number1) : LameMob(position1, number1)
{
    ...
    if(armored)
    {
        ...
        else if (armorChance >= 37 && armorChance < 85)
        {
            helmetId = GoldenHelmet::getID();
        }
        else if (armorChance >= 85 && armorChance < 98)
        {
            helmetId = ChainmailHelmet::getID();
        }
        else if (armorChance >= 98)
        {
...
void Zombie::BoxCollision()
{
    for(int x = 0; x < world->mZombies.size(); x++)
    {
        ...
        if(TestZombie2->number != number && (TestZombie2->position.x != position.x || TestZombie2->position.z != position.z || TestZombie2->position.y != position.y))
        {
            ...
            {
                float angle1 = ATAN2(position.x-TestZombie2->position.x,position.z-TestZombie2->position.z); // super secret function
            ...
            if(dt < 0.1f)
            {
                    ...
                    if(kickTimer >= 1.0f)
                    {
                        ...
                        if(world->BlockSolid(position.x+checkPos.x,position.y+checkPos.y,position.z+checkPos.z) == false && world->BlockSolid(position.x+checkPos2.x,position.y+checkPos2.y,position.z+checkPos2.z) == false && world->BlockSolid(position.x+checkPos3.x,position.y+checkPos3.y,position.z+checkPos3.z) == false)
                        {
                            if(world->HP > 0)
                            {
                                ...
                                world->kickedAngle = ATAN2(position.x-myWorld->playerPos.x,position.z-myWorld->playerPos.z);
                                world->kickedBy = 1;
                ...
                if(killed == false)
                {
                    ...
                    else // it doesn't see the player
                    {
                        ...
                        if(hauntingTimer >= hauntingTimerMax)
                        {
                            ...
                            if(rand()%100 < 80) // walk to some point nearby
                            {
                                ...
                                hauntingAngle = ATAN2(position.x-hauntingPos.x,position.z-hauntingPos.z)+PI;
                            }
                    ...
                    if(angred == true && kicked == false)
                    {
                        ...
                        float playerAngle = ATAN2(position.x-relPlayerPos.x,position.z-relPlayerPos.z)+PI;
                ...
                if(kicked == true)
                {
                    mainAngle = ATAN2(position.x-relPlayerPos.x,position.z-relPlayerPos.z)+PI; // super secret function
                    TryToMoveWithoutJump(Vector3(sinf(mainAngle+PI),0.0f,cosf(mainAngle+PI)),(SPD+0.18f)*9.2f*(damagedTimer*damagedTimer)*dt);
