...
void StatePlay::LoadMap(std::string fileName,bool compressed)
{
    ...
    Utils::Logger::Instance()->LogMessage("[1]");
    ...
    Utils::Logger::Instance()->LogMessage("[2]");
        ...
        if(menuState == 0)//game state
        {
           ...
           /* if (mSystemMgr->KeyPressed(PSP_CTRL_RTRIGGER)) //open inventory
            ...
            if(keyPressed(InputHelper::Instance()->getButtonToAction(14)))
            {
                ...
                if(mWorld->invId[27+barPosition] == Scissors::getID() || mWorld->ItemType(mWorld->invId[27+barPosition]) == 'Y') // shear sheep or dye it
                {
                    ...
                    if(mWorld->BlockEditable(testPos.x,testPos.y,testPos.z) && testBox.contains(testPos))
                    {
                                ...
                                if(mWorld->GetBlock(testPos.x,testPos.y+1,testPos.z) == 0)
                                {
                                    ...
                                    if(TestJukebox->GetRecord() == -1)
                                    {
                                        if(mWorld->invId[27+barPosition] >= Disk1::getID() && mWorld->invId[27+barPosition] <= Disk3::getID())
                                        {
                                            ...
                                            mSoundMgr->StopDiskSounds();
                                            mSoundMgr->PlayDiskSound(mWorld->invId[27+barPosition]-Disk1::getID());

                                            switch(mWorld->invId[27+barPosition]-Disk1::getID())
                                            {
                                                case 0: inputDiskName = "C418 - 13"; break;
                                                case 1: inputDiskName = "C418 - cat"; break;
                                                case 2: inputDiskName = "C418 - strad"; break;
                                            }
                           ...
                           )
                        {
                            ...
                            if(testPos.y - y > 0.4f)
                            {
                                ...
                                BoundingBox playerBox = BoundingBox(Vector3(playerPosition.x - 0.15f,playerPosition.y - 1.63f,playerPosition.z - 0.15f),Vector3(playerPosition.x + 0.15f,playerPosition.y + 0.2f,playerPosition.z + 0.15f));
                        ...
                        if(selectedBlock >= BedBlock1::getID() && selectedBlock <= BedBlock8::getID())
                        {
                            ...
                            if(mWorld->worldDayTime < 6 || mWorld->worldDayTime > 21)
                            {
                                mWorld->playerSpawnPointPosition = mWorld->playerPos;
                                sleepTime = 3.0f;
                            }
                        }
                        ...
                        else
                        {
                            ...
                            if(upper_block != 0)
                            {
                                ...
                                for(int checkY = testPos1.y+1; checkY <= testPos1.y+5; checkY++)
                                {
                                    ...
                                    if(mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) != 33 && mWorld->GetBlock(testPos2.x,testPos2.y,testPos2.z) != CactusBlock::getID())
                                    {
                                        if(mWorld->ChunksEqual(testPos1, testPos2) == false)
                                        {
                                            int chunkTarget = mWorld->getChunkId(testPos2);
    ...
    if(mWorld->HP <= 0)
    {
        ...
        if(mSystemMgr->KeyPressed(PSP_CTRL_CROSS))
        {
            ...
            else
            {
                ...
                {
                    newPosition.y = mWorld->groundHeight(newPosition.x,newPosition.z)+1.7f;
                }
...
void StatePlay::Update(StateManager* sManager)
{
    ...
    if(menuState == 0)//game state
    {
                ...
                if(playerBox.intersect((*it)->actArea))
                {
                    ...
                    if((*it)->ItIsTimeToSpawn() && mWorld->BlockFinalLight((*it)->GetX(),(*it)->GetY()+1,(*it)->GetZ()) < 0.6f && mWorld->mainOptions.difficult != 0)
                    {
                        ...
                        if((*it)->MobToSpawn() == "zombie")
                        {
                            ...
                            if(nearbyEntityAmount < 4)
                            {
                                for(int i = 0; i < (*it)->MobCountToSpawn(); i++)
                                {
        ...
        if (mWorld->skyTime >= 273 && mWorld->skyTime <= 288)
        {
            mWorld->dawnSunsetFactor = (mWorld->skyTime - 273)/18.0f;
        }
        ...
        if(dt < 0.1f)
        {
            ...
            if(mWorld->SolidAtPointForPlayer(footPosition) || mWorld->SolidAtPointForPlayer(headPosition))
            {
                ...
                if (mWorld->SolidAtPointForPlayer(headPosition))
                {
                    ...
                    playerPosition.y = (float)(blockIn - 0.25f); //0.15
                }

                // If the player is stuck in the ground, bring them out.
                // This happens because we're standing on a block at -1.5, but stuck in it at -1.4, so -1.45 is the sweet spot.
                if (mWorld->SolidAtPointForPlayerOnGround(footPosition))
                {
                    int blockOn = (int)(footPosition.y);

                    if(mWorld->GetBlock(footPosition.x,footPosition.y,footPosition.z) == CactusBlock::getID())
                    {
                        touchingCactus = true;
                    }

                    if(mWorld->PlayerOnHalfBlock(footPosition))
                    {
                        playerPosition.y = (float)(blockOn + 1 + 1.1f);
                    }
                    else if(mWorld->PlayerOnStairsBlock(footPosition))
                    {
                        if(mWorld->PlayerOnHighStair(footPosition))
                        {
                ...
                {
                    if (!mWorld->PlayerInWater(Vector3(footPosition.x,footPosition.y+0.45f,footPosition.z)) && mWorld->GetBlock(playerPosition.x,playerPosition.y-1.3f,playerPosition.z) != 87 && mWorld->GetBlock(playerPosition.x,playerPosition.y-1.3f,playerPosition.z) != 88 && mWorld->GetBlock(playerPosition.x,playerPosition.y-1.3f,playerPosition.z) != 89 && mWorld->GetBlock(playerPosition.x,playerPosition.y-1.3f,playerPosition.z) != 90)
                    {
                        if(playerVelocity.y < -6 && rand() % 10 >= 7) // destroy soil
                        {
                            if(mWorld->GetBlock(footPosition.x, footPosition.y-0.25f, footPosition.z) == Soil::getID() || mWorld->GetBlock(footPosition.x, footPosition.y-0.25f, footPosition.z) == WateredSoil::getID())
                            {
                                mWorld->GetBlock(footPosition.x, footPosition.y-0.25f, footPosition.z) = DirtBlock::getID();
                                if(mWorld->GetBlock(footPosition.x, footPosition.y+0.25f, footPosition.z) != 0)
                                {
                                    mWorld->DropThis(mWorld->BlockLoot(footPosition.x, footPosition.y+0.25f, footPosition.z),1,true,Vector3(footPosition.x, footPosition.y+0.25f, footPosition.z));
                                    mWorld->GetBlock(footPosition.x, footPosition.y+0.25f, footPosition.z) = 0;
                                }
                                if(!mWorld->ChunksEqual(Vector3(footPosition.x, footPosition.y-0.25f, footPosition.z), Vector3(footPosition.x, footPosition.y+0.25f, footPosition.z)))
                                {
                                    mWorld->RebuildFullMeshChunk(mWorld->getChunkId(Vector3(footPosition.x, footPosition.y-0.25f, footPosition.z)));
                                    mWorld->RebuildFullMeshChunk(mWorld->getChunkId(Vector3(footPosition.x, footPosition.y+0.25f, footPosition.z)));
                                }
                                else
                                {
            ...
            if(mWorld->kickedStart)
            {
                if(mWorld->kickedBy == 1) // if you damaged by zombie
                {
                    ...
                    break;
                    case 1: // easy
                        HurtPlayer(mWorld->kickedDamage-1);
                    break;
                    case 2: // normal
                        HurtPlayer(mWorld->kickedDamage);
                    break;
                    case 3: // hard
                        HurtPlayer(mWorld->kickedDamage+1);
                    break;
                ...
                if(mWorld->kickedBy == 2) // creeper or TNT
                {
                    ...
                    break;
                    case 1: // easy
                        HurtPlayer(mWorld->kickedDamage*0.66f);
                    break;
                    case 2: // normal
                        HurtPlayer(mWorld->kickedDamage);
                    break;
                    case 3: // hard
                        HurtPlayer(mWorld->kickedDamage*1.33f);
                    break;
                    }
            ...
            if(moveVector.x != 0.0f || moveVector.z != 0.0f)
            {
                ...
                {
                    isWalking = true;
                    if(mWorld->mainOptions.autoJump == true)
                    {
                        jumping = true;
                    }
                }
...
void StatePlay::Draw(StateManager* sManager)
{
    ...
    if(mWorld->mainOptions.detailedSky == 1)
    {
        ...
        {
            Vector3 color = ((mWorld->startSkyColor*mWorld->brightFactor)*nullFactor+(mWorld->dawnSunsetSkyColor*mWorld->dawnSunsetFactor*angleFactor))*(playerPosition.y/48.0f)*(playerPosition.y/48.0f);
            sceGuFog(mWorld->mainOptions.horizontalViewDistance * 14, mWorld->mainOptions.horizontalViewDistance * 14 + 145.0f, GU_COLOR(color.x,color.y,color.z,1.0));
        }
        else
        {
            Vector3 color = (mWorld->startSkyColor*mWorld->brightFactor)*nullFactor+(mWorld->dawnSunsetSkyColor*mWorld->dawnSunsetFactor*angleFactor);
        ...
        /// Skybox rendering
        {
            ...
            {
                skyBox->Render(mWorld->endSkyColor*mWorld->brightFactor*(playerPosition.y/45.0f));
            }
            else
            {
                skyBox->Render(mWorld->endSkyColor*mWorld->brightFactor);
            }
            MatrixPop();
        }
        sceGuDisable(GU_FOG);
        ...
        {
            MatrixColor(GU_COLOR(1.0f,1.0f,1.0f,1-mWorld->rainyColorAlpha));
        ...
        if(mWorld->rainyColorAlpha != 1.0f)
        {
            if(mWorld->mainOptions.fogRendering == 1) /// FOG FOR CLOUDS
            {
                ...
                else
                {
                    ...
                    {
                        Vector3 color = (( mWorld->startSkyColor*mWorld->brightFactor)*nullFactor+(mWorld->dawnSunsetSkyColor*mWorld->dawnSunsetFactor*angleFactor))*(playerPosition.y/48.0f)*(playerPosition.y/48.0f);
                        sceGuFog(mWorld->mainOptions.horizontalViewDistance * 14 + 70.0f, mWorld->mainOptions.horizontalViewDistance * 14 + 110.0f, GU_COLOR(color.x,color.y,color.z,1.0));
                    }
                    else
                    {
                        Vector3 color = (mWorld->startSkyColor*mWorld->brightFactor)*nullFactor+(mWorld->dawnSunsetSkyColor*mWorld->dawnSunsetFactor*angleFactor);
            ...
            MatrixTranslation(Vector3(-114,112,-114));
            ...
            if(d < mWorld->mZombies.size() && mWorld->mZombies.empty() == false)
            {
                ...
                if(TestZombie->toDestroy == true || (TestZombie->killed == true && TestZombie->deathTimer <= 0.0f))
                {
                    if(TestZombie->killed == true)
                    {
                        ...
                        if(TestZombie->helmetId != -1 && rand() % 2 == 1)
                        {
                        ...
                        if(TestZombie->chestplateId != -1 && rand() % 2 == 1)
                        {
                        ...
                        if(TestZombie->leggingsId != -1 && rand() % 2 == 1)
                        {
                        ...
                        if(TestZombie->bootsId != -1 && rand() % 2 == 1)
                        {
            ...
            if(d < mWorld->mCreepers.size() && mWorld->mCreepers.empty() == false)
            {
                ...
                if(TestCreeper->toDestroy == true || (TestCreeper->killed == true && TestCreeper->deathTimer < 0.0f))
                {
                    if(TestCreeper->killed == true)
                    {
                        ...
                        if(rand() % 40 == 0) // 2 % of disk dropping
                        {
                            mWorld->DropThis(Disk1::getID()+rand()%3,1,false,Vector3(TestCreeper->position.x,TestCreeper->position.y+0.3f,TestCreeper->position.z));
                        }
                    }
	...
	if(mWorld->mainOptions.guiDrawing == 1 && mWorld->HP > 0)
	{
        ...
        if(mWorld->invId[27+barPosition] >= 250)
        {
            ...
            //translate
            ScePspFVector3 move = {0.735f+cubeBob+shift_x+(mWorld->mainOptions.fov-70)/250.0f+sinf(animDest)*-0.25,-0.24f+cubeBob2+shift_y+changeY,-0.6f+(mWorld->mainOptions.fov-70)/180.0f};//-0.17
	...
	if ((invEn == false && craft3xEn == false && chestEn == false && furnaceEn == false && menuState == 0 && mWorld->mainOptions.guiDrawing == 1) || makeScreen == true)
    {
        ...
        if (menuState != 1 || makeScreen == true)
        {
            ...
            for(int i = 1; i <= 10; i++)
            {
                hpCellSpriteW->SetPosition(-4+i*16,12);
                hpCellSprite->SetPosition(-4+i*16,12);
                ...
                hgCellSprite->SetPosition(326+160-i*16,12);

                hgSprite->SetPosition(326+160-i*16,12);
                hgHalfSprite->SetPosition(326+160-i*16,12);
                ...
                arCellSprite->SetPosition(-4+i*16,32);
                ...
                if(headInWater)
                {
                    bubbleSprite->SetPosition(326+160-i*16,32);
                    if(i <= mWorld->OS)
                    {
                        bubbleSprite->Draw();
                    }
        ...
        for(int k = 0; k <= 8; k++)
        {
            if(mWorld->invId[27+k] != -1)
            {
                ...
                if(mWorld->invId[27+k] >= 250)
                {
                    ...
                    if(mWorld->invId[27+k] <= 276 || mWorld->invId[27+k] == Clock::getID() || mWorld->invId[27+k] == Compass::getID() || mWorld->invId[27+k] == FlintAndSteel::getID())
                    {
    ...
    if(menuState == 1)
    {
        if(menuOptions)
        {
            ...
            if(language == ENGLISH)
            {
                ...
                mWorld->mainOptions.worldBlockAnimation == true ? mRender->DebugPrint(110,199,"Animated Blocks: ON"): mRender->DebugPrint(110,199,"Animated Blocks: OFF");
                ...
                mWorld->mainOptions.autoJump == true ? mRender->DebugPrint(110,229,"Auto-jump: ON"): mRender->DebugPrint(110,229,"Auto-jump: OFF");
                ...
                mWorld->mainOptions.smoothLighting == true ? mRender->DebugPrint(110,259,"Smooth Lighting: ON"): mRender->DebugPrint(110,259,"Smooth Lighting: OFF");
            ...
            if(language == RUSSIAN)
            {
                ...
                optionsMenuPos == 4 ? RenderManager::InstancePtr()->SetFontStyle(default_size,GU_COLOR(1,1,0,1),0,0x00004000|0x00000200) : RenderManager::InstancePtr()->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00004000|0x00000200);
                mWorld->mainOptions.fastRendering == true ? mRender->DebugPrint(110,169,"Grafika: Skorost$"): mRender->DebugPrint(110,169,"Grafika: Kayestvo");

                optionsMenuPos == 5 ? RenderManager::InstancePtr()->SetFontStyle(default_size,GU_COLOR(1,1,0,1),0,0x00004000|0x00000200) : RenderManager::InstancePtr()->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00004000|0x00000200);
                mWorld->mainOptions.worldBlockAnimation == true ? mRender->DebugPrint(110,199,"Animaci^ blokov : Vkl"): mRender->DebugPrint(110,199,"Animaci^ blokov: V@kl");

                optionsMenuPos == 6 ? RenderManager::InstancePtr()->SetFontStyle(default_size,GU_COLOR(1,1,0,1),0,0x00004000|0x00000200) : RenderManager::InstancePtr()->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00004000|0x00000200);
                mWorld->mainOptions.autoJump == true ? mRender->DebugPrint(110,229,"Avto-pr@jok: Vkl"): mRender->DebugPrint(110,229,"Avto-pr@jok: V@kl");

                optionsMenuPos == 7 ? RenderManager::InstancePtr()->SetFontStyle(default_size,GU_COLOR(1,1,0,1),0,0x00004000|0x00000200) : RenderManager::InstancePtr()->SetFontStyle(default_size,GU_COLOR(1,1,1,1),0,0x00004000|0x00000200);
                mWorld->mainOptions.smoothLighting == true ? mRender->DebugPrint(110,259,"M^gkoe osvexenie: Vkl"): mRender->DebugPrint(110,259,"M^gkoe osvexenie: V@kl");
...
bool StatePlay::TryToMove(Vector3 moveVector,float dt)
{
    ...
    Vector3 midBodyPoint = movePosition + Vector3(0, -0.55f, 0);
    Vector3 midLowBodyPoint = movePosition + Vector3(0, -1.05f, 0);
    Vector3 lowerBodyPoint = movePosition + Vector3(0, -1.55f, 0);

    if (!mWorld->SolidAtPointForPlayer(movePosition) && !mWorld->SolidAtPointForPlayer(lowerBodyPoint) && !mWorld->SolidAtPointForPlayer(midBodyPoint) && !mWorld->SolidAtPointForPlayer(midLowBodyPoint))
    {
    ...
    midBodyPoint = movePosition + Vector3(0, -0.55f, 0);
    midLowBodyPoint = movePosition + Vector3(0, -1.05f, 0);
